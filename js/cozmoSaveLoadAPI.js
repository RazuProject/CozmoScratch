(function () {
    ...
    window.remixProject = function(projectUUID, newProjectName) {
        ...
        // Special case: if we are in a horizontal sample project (which is likely to be in old XML), send along the serialized JSON to use for the remix.    
        var serializedJSON = null;
        if (!window.isVertical && window.isCozmoSampleProject) {            
            serializedJSON = window.Scratch.vm.toJSON();
        }

        window.Unity.call({requestId: -1, command: "cozmoRequestToRemixProject", argUUID: projectUUID, argString: newProjectName, argString2: projectType, argString3: serializedJSON});
    }
