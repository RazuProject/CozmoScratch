(function () {
    ...
    /**
     * This takes a Code Lab project JSON and tries to localize it dynamically
     * by searching for localization keys starting with the string 'codeLabFeaturedProject'
     * and replacing those instances with text appropriate for our current locale.
     * The current locale, and the translation data, is loaded and handled by localize.js
     *
     * @param {object} project - The JSON project, or string representation
     * @returns {object} - The translated JSON project, in the same format originally recieved
     */
    function localizeCodeLabProjectJSON(project) {
        // Ensure we're working with a string representation of the JSON
        var returnAsString = true;
        var stringRequest = project;
        if (typeof stringRequest != 'string') {
            returnAsString = false;
            stringRequest = JSON.stringify(stringRequest);
        }
        // This regex finds any keys that need to be looked up
        var re = /\"codeLabFeaturedProject\.(.*?)\"/g;
        // Apply the regex replacement
        stringRequest = stringRequest.replace(
            re,
            function (m, key) {
                // Rebuild the full key, without enclosing quotes
                var localizationKey = 'codeLabFeaturedProject.' + key;
                var result = localizationKey;
                if (localizationKey in window.codeLabFeaturedTranslations) {
                    result = window.codeLabFeaturedTranslations[localizationKey]['translation'];
                    result = result.replace("\n", "\\n");
                } else {
                    console.log("Unhandled Localization Key: " + result);
                }
                // Return the localized result, within quotes, to preserve JSON formatting
                return '"' + result + '"';
            }
        );
        ...
        return JSON.parse(stringRequest);
    }
    ...
    window.openCozmoProjectJSON = function (cozmoProjectJSON) {
        ...
        try {
            cozmoProjectJSON.projectJSON = localizeCodeLabProjectJSON(cozmoProjectJSON.projectJSON);
    ...
    window.openCozmoProject = function(projectUUID, projectName, projectVersionNum, projectJSON, projectXML, isCozmoSampleProjectStr) {
        ...
        if (projectJSON != null) {
            window.Scratch.vm.loadProject(JSON.stringify(projectJSON));
        }
        else {
    ...
    // This prevents "Remix Project" prompt from appearing after only running Featured or Sampled projects,
    // without actually making any changes. https://ankiinc.atlassian.net/browse/COZMO-15521
    //
    // Returns project as string.
    function cleanSerializedProjectStringForComparison(serializedProjectAsString) {
        if (serializedProjectAsString != null) {
            var serializedProjectJSONObject = JSON.parse(serializedProjectAsString);

            // We check the VM version in the serialized project,
            // so we can clear variables in the correct way
            // In the August 2018 Scratch Merge, the VM serialization
            // format changed.
            // VM 0.1.0 - pre August 2018 merge
            // VM 0.2.0 - post August 2018 merge
            var vmVersion = serializedProjectJSONObject.meta.vm;

            if (serializedProjectJSONObject.targets[0]) {
                var target = serializedProjectJSONObject.targets[0];
                for (var key in target.variables) {
                    if (target.variables.hasOwnProperty(key)) {
                        var variable = target.variables[key];
                        switch (vmVersion) {
                            case "0.1.0":
                                variable.value = "";
                                break;
                            default:
                                // This format change was introduced in VM 0.2.0
                                variable[1] = "";
                                break;
                        }
                    }
                }
                // VM 0.2.0 introduced a new variable category - lists
                if (target.lists) {
                    for (var key in target.lists) {
                        if (target.lists.hasOwnProperty(key)) {
                            var list = target.lists[key];
                            list[1] = [];
                        }
                    }
                }
